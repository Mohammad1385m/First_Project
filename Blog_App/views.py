...
class Blog_Detail_View(View):
    ...
    # def post(self, request, slug):
        ...
        # """*************************un-commenting the next line will send notif with success*************************"""
        # messages.success(request, "کامنت شما بعد از تایید اضافه خواهد شد")

        # return redirect(reverse("blog_detail", args=[slug]))

        # return redirect("send_comment")
...
def send_comment(request):
    if request.method == "POST":
        try:
            ...
            if blog_id and text:
                new_comment = Blog_Comment(user=request.user, blog_id=blog_id, text=text)
                new_comment.save()
                comments = Blog_Comment.objects.filter(blog_id=blog_id, is_active=True).order_by("-create_at")

                return render(request, "Comment_Template.html",{
                    "comments": comments
                })
